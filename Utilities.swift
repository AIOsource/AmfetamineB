...
// MARK: - App Launcher via LSApplicationWorkspace (Erosion method)

@discardableResult
func openApp(withBundleID bundleID: String) -> Bool {
    // 1. C helper with dynamic runtime LSApplicationWorkspace invocation
    if LaunchApplicationWithBundleID(bundleID) == 1 {
        return true
    }

    // 3. Fallback URL schemes for MobilePhone
    if bundleID == "com.apple.mobilephone" {
        for scheme in ["mobilephone://", "mobilephone-recents://", "tel:"] {
...
// MARK: - Loopback VPN Launcher

func connectLoopbackVPN() {
    LoopbackVPNManager.shared.connect()
}

func disconnectLoopbackVPN() {
    LoopbackVPNManager.shared.disconnect()
}
