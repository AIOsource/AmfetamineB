...
enum AppThemeChoice: String, CaseIterable, Identifiable {
    case pureBlack = "Pure Black (OLED)"
    case dark = "Dark"
    case light = "Light"
    case system = "System"
    ...
    var colorScheme: ColorScheme? {
        switch self {
        case .pureBlack, .dark: return .dark
...
enum AppAccentColor: String, CaseIterable, Identifiable {
    ...
    case purple = "Cyber Purple"
    case orange = "Vibrant Orange"
    case white = "Monochrome White"
...
struct RemotePasscodeTheme: Identifiable, Codable, Hashable {
    var id: String { url }
    let name: String
    ...
    var authorString: String {
        if let contact = contact, !contact.isEmpty {
            return contact.map { "\($0.key): \($0.value)" }.joined(separator: " · ")
        }
        return ""
    }

    var previewURL: URL? {
        let base = "https://raw.githubusercontent.com/sourcelocation/Cowabunga-theme-repo/main/"
        if preview.hasPrefix("http") {
            return URL(string: preview)
        }
        return URL(string: base + preview)
    }
