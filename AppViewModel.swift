...
final class AppViewModel: ObservableObject {
    ...
    @Published var remotePasscodeThemes: [RemotePasscodeTheme] = []
    @Published var isFetchingPasscodeThemes: Bool = false
    @Published var passcodeThemesFetchError: String? = nil
    @Published var downloadingThemeUrl: String? = nil
    ...
    func importPairingFile(from sourceURL: URL, originalName: String? = nil) -> Bool {
        ...
        do {
            ...
            refreshPairingFile()
            pairingStatus = "Pairing file loaded [OK] (\(originalName ?? "aircard_pairing.plist"))"
            return true
        } catch {
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    self.pairingStatus = "Paired successfully! [OK]"
    ...
    func refreshNetworkStatus() {
        ...
        let inAppVPNActive = LoopbackVPNManager.shared.isEnabled
        vpnUp = vpn || inAppVPNActive
        wifiUp = wifi
        ...
        Task.detached { [weak self] in
            ...
            for (i, card) in selected.enumerated() {
                ...
                guard let sourceImg = await MainActor.run(body: { card.customImage ?? card.customImageData.flatMap { UIImage(data: $0) } }) else {
                    await MainActor.run { self.cardFlashLog.append("  [!] No image for card \(card.id.prefix(8))") }
                    continue
                }
                ...
                guard !allSkins.isEmpty else {
                    await MainActor.run { self.cardFlashLog.append("  [!] Failed to generate card skins") }
                    continue
                }
            ...
            await MainActor.run {
                if successCount > 0 {
                    ...
                    self.cardFlashLog.append("[Done] \(successCount)/\(selected.count) card(s) flashed! Force-close Wallet app to see changes.")
    ...
    // MARK: - Remote Passcode Themes (Explore)

    func fetchRemotePasscodeThemes() {
        if remotePasscodeThemes.isEmpty {
            if let path = Bundle.main.path(forResource: "fallback_passcode_themes", ofType: "json"),
               let data = try? Data(contentsOf: URL(fileURLWithPath: path)),
               let themes = try? JSONDecoder().decode([RemotePasscodeTheme].self, from: data) {
                self.remotePasscodeThemes = themes
            }
        }

        isFetchingPasscodeThemes = true
        passcodeThemesFetchError = nil
        Task {
            do {
                guard let url = URL(string: "https://raw.githubusercontent.com/sourcelocation/Cowabunga-theme-repo/main/passcode-themes.json") else {
                ...
                await MainActor.run {
                    self.remotePasscodeThemes = themes
                    self.isFetchingPasscodeThemes = false
                }
            } catch {
                await MainActor.run {
                    if self.remotePasscodeThemes.isEmpty {
                        self.passcodeThemesFetchError = error.localizedDescription
                    }
                    self.isFetchingPasscodeThemes = false
                }
            }
    ...
    func downloadAndApplyRemoteTheme(_ theme: RemotePasscodeTheme, autoFlash: Bool = false) {
        guard let downloadURL = theme.downloadURL else { return }
        downloadingThemeUrl = theme.url
        Task {
            do {
                ...
                let docs = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
                let cleanFileName = (theme.url as NSString).lastPathComponent
                let localURL = docs.appendingPathComponent(cleanFileName)
                try data.write(to: localURL)
                
                await MainActor.run {
                    self.downloadingThemeUrl = nil
                    self.scanDocumentsDirectory()
                    self.loadPassthm(url: localURL)
                    if autoFlash && self.hasPairingFile && self.vpnUp {
                        self.flashPassthm()
                    }
                }
            } catch {
        ...
        Task.detached { [weak self] in
            ...
            for (idx, targetPath) in targetDirs.enumerated() {
                ...
                if !stepOk {
                    allOk = false
                    await MainActor.run {
                        self.passthmFlashLog.append("  [!] Write to \(targetName) failed: \(lastErr ?? "error")")
                    }
                } else {
                    await MainActor.run {
            ...
            await MainActor.run {
                if allOk {
                    ...
                    self.passthmFlashLog.append("[Done] Passcode theme applied! Lock your iPhone to see it.")
    ...
    // MARK: - Respring Actions

    func respring() {
        log.append("[*] Triggering device respring (Bad-Poster Mond & XPC w00t)...")
        UIImpactFeedbackGenerator(style: .heavy).impactOccurred()
        RespringHelper.respring { [weak self] in
            self?.log.append("[+] Respring dispatched. SpringBoard reloading...")
        }
    }

    func respringBadQuery() {
        respring()
    }
}
